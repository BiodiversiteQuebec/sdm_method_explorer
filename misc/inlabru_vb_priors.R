...
sp <- "Picoides arcticus"
year <- 2017
y <- year

obs <- d[scientific_name == sp & year %in% y, ]
...
s <- d[year %in% y & month_day >= min(md) & month_day <= max(md)]
...
sds <- c(0.0001, 0.001, 0.002, 0.005, 0.0075, 0.01, 0.015, 0.02, 0.025, 0.03, 0.05, 0.1, 0.2, 0.5, 2)
spatialsd <- NULL
coefs <- vector(mode = "list", length = length(sds))

for(i in sds){

matern <-
  inla.spde2.pcmatern(mesh,
                      prior.sigma = c(i, 0.01), # 0.02 - 0.005
                      prior.range = c(100000, 0.01)
  )
...
spatialsd <- c(spatialsd, fit$summary.hyperpar[2,1])

coefs[[match(i, sds)]] <- rbind(fit$summary.fixed[, names(fit$summary.hyperpar)], fit$summary.hyperpar)


}

plot(sds, spatialsd, type = "b", log = "", xlab = "prior spatial sd ", ylab = "spatial sd posterior mean", xlim = c(0,0.5))

coefs <- do.call("rbind",lapply(coefs, function(x){
  t(x[, 1, drop = FALSE])
}))
coefs <- cbind(sds = sds, coefs)
coefs <- head(coefs, -1)
coefs <- as.data.frame(coefs)

par(mfrow=c(ncol(coefs)-1,1), mar=c(3,3,0,0))
lapply(names(coefs)[-1], function(i){
  plot(coefs$sds, coefs[[i]], type = "b", log = "", xlab = "prior spatial sd ", ylab = "spatial sd posterior mean")
  mtext(side = 3, adj = 0.98, line = -1.2, text = i, cex = 0.6, font = 2)
})

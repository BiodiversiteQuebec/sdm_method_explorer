...
add_range <- function(file, add = TRUE){
  if(exists("aires")){
    x <- strsplit(file, "_")[[1]][2:3] |> paste(collapse = " ")
    w <- which(tolower(aires$species) == x)    
    if(any(w)){
      if(add){
        plot(st_geometry(aires[w, ]), border = NA, col = adjustcolor("pink", 0.15), add = add)
      } else {
        st_geometry(aires[w, ])
      }
    } else {
